...
from data.misc.resnet_utils import myResnet
import data.misc.resnet
...
class DataLoaderRaw():
    ...
    def __init__(self, opt):
        ...
        self.my_resnet = getattr(data.misc.resnet, self.cnn_model)()
    ...
    def get_batch(self, split, batch_size=None):
        ...
        att_batch = np.ndarray((batch_size, 49, 2048), dtype = 'float32')
        max_index = self.N
        wrapped = False
        infos = []
        for i in range(batch_size):
            ...
            tmp_fc, tmp_att = self.my_resnet(img,7)
            tmp_att = torch.reshape(tmp_att, (-1, 2048))
